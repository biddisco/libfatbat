...
public:
    ...
    ~controller_base()
    {
      ...
      [[maybe_unused]] unsigned int messages_handled_ = sends_posted_.value_ +
          recvs_posted_.value_ + reads_posted_.value_ + writes_posted_.value_;
    ...
    // --------------------------------------------------------------------
    int poll_send_queue_default(fid_cq* tx_cq, void* user_data)
    {
      // LIBFATBAT_SCOPE(bctrl_log, "{} tx_cq {} {}", (void*) (this), (void*) (tx_cq), __func__);
      ...
      assert(max_completions_per_poll_ <= max_completions_array_limit_);
      {
        ...
        // LF_DEB(cnt_deb<9>, timed(polling, static_cast<void*>(tx_cq)));
        ...
        if (ret == -FI_EAVAIL)
        {
          ...
          OperationContext* handler = reinterpret_cast<OperationContext*>(e.op_context);
      ...
      if (ret > 0)
      {
        ...
        for (int i = 0; i < ret; ++i)
        {
          ...
          {
            LIBFATBAT_ERROR(
                bctrl_log, "Received an unknown txcq completion, flags {:016b}", entry[i].flags);
            std::terminate();
          }
        }
        return processed;
      ...
      {
        LIBFATBAT_ERROR(bctrl_log, "{:<20} unknown error in completion txcq read", "Completion");
      }
      return 0;
    }

    // --------------------------------------------------------------------
    int poll_recv_queue_default(fid_cq* rx_cq, void* user_data)
    {
      // LIBFATBAT_SCOPE(bctrl_log, "{} rx_cq {} {}", (void*) (this), (void*) (rx_cq), __func__);
#ifdef EXCESSIVE_POLLING_BACKOFF_MICRO_S
      std::chrono::steady_clock::time_point now = std::chrono::steady_clock::now();
      ...
      assert(max_completions_per_poll_ <= max_completions_array_limit_);
      {
        ...
        if (ret == -FI_EAVAIL)
        {
          ...
          {
            LIBFATBAT_DEBUG(bctrl_log,
                "{:<20} {:02}, rxcq Cancelled, flags {:#06x}, len {:#010x}, context {:12p}",
          ...
          {
            LIBFATBAT_ERROR(bctrl_log,
                "{:<20} {:02}, rxcq error, flags {:#06x}, len {:#010x}, context {:12p}, error msg "
                "{}",
                "Completion", 0, e.flags, e.len, (void*) (e.op_context),
      ...
      {
        LIBFATBAT_ERROR(bctrl_log, "{:<20} unknown error in completion rxcq read", "Completion");
      }
      return 0;
    }
