...
struct region_provider
{
	...
	// register region
	static inline int fi_register_memory(provider_domain *pd,
	                                     int              device_id,
	                                     void const      *buf,
	                                     size_t           len,
...
struct memory_handle
{
	...
	memory_handle(memory_handle &&other) noexcept
	    : address_{other.address_}, region_{std::exchange(other.region_, nullptr)},
	      size_{other.size_}, used_space_{other.used_space_}
	{}
	...
	inline unsigned char *get_address(void) const
	{
		return address_;
	}
...
struct memory_segment : public memory_handle
{
	...
	// --------------------------------------------------------------------
	memory_segment(provider_region *region,
	...
	memory_segment(memory_segment &&other) noexcept
	    : memory_handle(std::move(other)), base_addr_{std::exchange(other.base_addr_, nullptr)}
	{}
	...
	memory_segment(provider_domain *pd,
	               void const      *buffer,
	               ...
	               int              device_id)
	{
		...
		if(bind_mr)
		{
			...
			{
				LF_DEB(NS_MEMORY::mrn_deb, trace(str<>("Bound region"), *this));
			}
			...
			{
				LF_DEB(NS_MEMORY::mrn_deb, trace(str<>("Enabled region"), *this));
			}
		}
	}
	...
	inline unsigned char *get_base_address(void) const
	{
		return base_addr_;
	}
